...
func catchError(err error) {
	...
	for _, id := range ids {
		bot.SendMessage(id, "Произошла ошибка в функции '"+callerName+"':\n"+err.Error())
	}
}
...
func addInNewGroup(update tgbotapi.Update) {
	...
	if flag {
		bot.SendMessage(int(update.Message.Chat.ID), "Here we go again")
		bot.SendMessage(int(update.Message.Chat.ID), `Команды, доступные только админам:
...
func detectYoungHacker(update tgbotapi.Update) {
	bot.SendMessage(update.Message.From.ID, "По ФЗ-35 гл. 23 за попытку доступа к приватным функциям вы приговариваетесь к бану.")
	ids, err := DB.GetGroupIDs()
	if err != nil {
		catchError(err)
	}
	for _, id := range ids {
		bot.SendMessage(id, "Пользователь с ID "+fmt.Sprint(update.Message.From.ID)+" и ником @"+fmt.Sprint(update.Message.From.UserName)+" попытался использовать функционал, на который у него нет прав:")
		if update.Message.NewChatMembers == nil && !update.Message.GroupChatCreated {
			bot.SendForward(id, int(update.Message.Chat.ID), update.Message.MessageID)
		} else {
			bot.SendMessage(id, "Добавление в чат.")
		}
	}
}
		...
		if update.Message != nil {
			...
			if update.Message.IsCommand() {
				...
				if command == "start" {
					...
					if status != -2 {
						bot.SendMessage(update.Message.From.ID, "Уже зарегистрирован")
					} else {
						...
						bot.SendMessage(update.Message.From.ID, question)
					}
					continue
				}
				flag, err := DB.IsAdmin(update.Message.From.ID)
				...
				case "send_message":
					if flag {
						...
						bot.SendMessage(int(update.Message.Chat.ID), "Запрос обработан")
					} else {
						detectYoungHacker(update)
					}
				case "get_parameter":
					if flag {
						...
						if len(arguments) == 2 {
							...
							bot.SendMessage(int(update.Message.Chat.ID), "Ожидается два аргумента")
						}
						bot.SendMessage(int(update.Message.Chat.ID), text)
						bot.SendMessage(int(update.Message.Chat.ID), "Запрос обработан")
					} else {
						detectYoungHacker(update)
					}
				case "add_admin":
					if flag {
						...
						if err != nil {
							...
							bot.SendMessage(int(update.Message.Chat.ID), "Админ с id "+argument+" успешно добавлен")
						}
						bot.SendMessage(int(update.Message.Chat.ID), "Запрос обработан")
					} else {
						detectYoungHacker(update)
				...
				case "delete_user":
					if flag {
						...
						if err != nil {
							...
							bot.SendMessage(int(update.Message.Chat.ID), "Пользователь с ID "+argument+" успешно удалён")
						}
						bot.SendMessage(int(update.Message.Chat.ID), "Запрос обработан")
					} else {
						detectYoungHacker(update)
				...
				case "add_question":
					if flag {
						arguments := strings.Fields(update.Message.CommandArguments())
						if len(arguments) < 2 {
							bot.SendMessage(int(update.Message.Chat.ID), "Ожидается хотя бы два аргумента")
						} else {
							question := ""
				...
				case "ban_user":
					if flag {
						...
						if err != nil {
							...
							bot.SendMessage(int(update.Message.Chat.ID), "Пользователь с ID "+argument+" успешно забанен")
						}
						bot.SendMessage(int(update.Message.Chat.ID), "Запрос обработан")
					} else {
						detectYoungHacker(update)
				...
				case "unban_user":
					if flag {
						...
						if err != nil {
							...
							bot.SendMessage(int(update.Message.Chat.ID), "Пользователь с ID "+argument+" успешно восстановлен в правах")
						}
						bot.SendMessage(int(update.Message.Chat.ID), "Запрос обработан")
					} else {
						detectYoungHacker(update)
				...
				status := DB.GetStatus(update.Message.From.ID)
				if status == -404 {
					bot.SendMessage(update.Message.From.ID, "Вы были заблокированы, обратитесь к админу для разблокировки")
				} else if status == -2 {
					bot.SendMessage(update.Message.From.ID, "Зарегистрируйтесь с помощью /start перед использованием")
				} else if status == -1 {
					ids, err := DB.GetGroupIDs()
					if err != nil {
						catchError(err)
					...
					for _, id := range ids {
						msg_id, err := bot.SendForward(id, int(update.Message.Chat.ID), update.Message.MessageID)
						if err != nil {
							catchError(err)
						}
					...
					if len(ids) > 0 {
						bot.SendMessage(update.Message.From.ID, "Вопрос отправлен")
					} else {
						bot.SendMessage(update.Message.From.ID, "Нет доступных чатов, обратитесь к администратору")
					}
					...
					bot.SendMessage(update.Message.From.ID, "Регистрация пройдена успешно")
					ids, err := DB.GetGroupIDs()
					if err != nil {
						catchError(err)
					}
					for _, id := range ids {
						bot.SendMessage(id, "Пользователь с ником @"+update.Message.From.UserName+" и ID "+fmt.Sprint(update.Message.From.ID)+" успешно зарегистрирован")
					}
				} else {
					parameter, err := DB.GetParameter(status)
					if err != nil {
